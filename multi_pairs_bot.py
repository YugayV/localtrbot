...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    input,select{width:100%;padding:10px;border:1px solid var(--border);border-radius:10px;background:var(--panel2);color:var(--text);outline:none}
...
<script>
let ltChart, eqChart;
function fmtMoney(v){const n=Number(v);return Number.isFinite(n)?('<html lang=\"en\">
<head>
  <meta charset=\"utf-8\" />
  <meta name=\"viewport\" content=\"width=device-width,initial-scale=1\" />
...
def get_longterm_series(pair_or_ticker):
    pair = str(pair_or_ticker or "").strip()
    if not pair:
        pair = "BTCUSD"
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            legacy = (qs.get("legacy", [""])[0] or "").strip()
            html = DASHBOARD_HTML if legacy else DASHBOARD2_HTML
...
async function loadAll(showMsg){
  try{
    ...
    setText('badge_updated', 'Updated: ' + new Date().toLocaleString());
